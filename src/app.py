...
class EEGApp(_BaseTk):
    ...
    def _show_help_dialog(self) -> None:
        ...
        tk.Label(
            header,
            text="Регистрация → выбор электродов → загрузка CSV → анализ → PDF",
            bg=UI["panel"], fg=UI["muted"], font=FONT_MAIN
        ).pack(anchor="w", pady=(6, 0))

        # --- body card with scrollable text ---
        ...
        add("1. Назначение программы\n", "h")
        add(
            "Программа предназначена для регистрации, визуализации и анализа ЭЭГ-сигналов "
            "с акцентом на λ-ритм (4–6 Гц). Поддерживается анализ одного или нескольких файлов "
            "и экспорт отчёта в PDF.\n\n",
            "sub"
        )

        add("2. Формат входных данных (CSV)\n", "h")
        add("• ", "bullet");
        add("Только CSV.\n", "bullet")
        add("• ", "bullet");
        add("Колонка времени — не обязательна.\n", "bullet")
        add("• ", "bullet");
        add("Одна или несколько числовых колонок сигналов — обязательны.\n", "bullet")
        add("• ", "bullet");
        add("Если времени нет — оно строится автоматически по FS.\n\n", "bullet")

        add("3. Вкладка «Онлайн»\n", "h")
        add("• ", "bullet");
        add("Выберите электроды во вкладке «Система 10–20».\n", "bullet")
        add("• ", "bullet");
        add("Выберите COM-порт и нажмите «Старт».\n", "bullet")
        add("• ", "bullet");
        add("Нажмите «Стоп» для паузы.\n", "bullet")
        add("• ", "bullet");
        add("Можно сохранить CSV или добавить запись в «Файлы».\n\n", "bullet")

        add("4. Вкладка «Система 10–20» (электроды)\n", "h")
        add("• ", "bullet");
        add("Можно выбрать до 3 электродов (или меньше, если данных/потоков меньше).\n", "bullet")
        add("• ", "bullet");
        add("Рекомендуется для λ-ритма: O1 / Oz / O2.\n", "bullet")
        add("• ", "bullet");
        add("Можно применить готовые наборы и нажать «Применить».\n\n", "bullet")

        add("5. Вкладка «Файлы»\n", "h")
        add("• ", "bullet");
        add("Добавление CSV через кнопку или drag&drop.\n", "bullet")
        add("• ", "bullet");
        add("Удаление выбранного файла или очистка списка.\n\n", "bullet")

        add("6. Вкладка «Анализ»\n", "h")
        add("Этапы расчёта:\n", "sub")
        steps = [
            "Чтение CSV и выбор каналов для анализа.",
            "Определение FS (берётся из поля FS, при необходимости уточняется по времени).",
            "Расчёт PSD (Welch) и мощности в диапазонах λ и α.",
            "Полосовая фильтрация λ (4–6 Гц).",
            "Расчёт мощности λ(t) в скользящем окне.",
            "Формирование текста «Анализ и выводы».",
        ]
        ...
        add("• ", "note");
        add("Если условие не выполняется — кнопка «Сравнение» отключается.\n\n", "note")

        add("8. Экспорт PDF\n", "h")
        add("Отчёт формируется в порядке:\n", "sub")
        add("• ", "bullet");
        add("Теория (определения + кратко).\n", "bullet")
        add("• ", "bullet");
        add("Анализ (интерпретация результатов).\n", "bullet")
        add("• ", "bullet");
        add("Таблицы.\n", "bullet")
        add("• ", "bullet");
        add("Графики.\n", "bullet")
        add("\n")
        add("Если сравнение недоступно — причина будет указана в отчёте.\n", "sub")
    ...
    def _build_1020_tab(self):
        ...
        ttk.Label(rec_card, text="Рекомендации по выбору электродов", style="H2.TLabel").pack(anchor="w")
        ttk.Label(
            rec_card,
            text=(
                "• Для регистрации λ-ритма рекомендуется использовать затылочные отведения: O1, Oz, O2, "
                "так как данный ритм связан со зрительной обработкой информации.\n"
                "• При отсутствии чисто затылочных электродов допускается использование теменно-затылочных "
                "отведений (например, Pz в сочетании с Oz или O2).\n"
                "• Лобные отведения (Fp1, Fp2, Fz) могут применяться в качестве контрольных каналов, "
                "однако они не являются приоритетными для анализа λ-ритма.\n\n"
            ),
            style="Muted.TLabel",
            justify="left",
            wraplength=620,
    ...
    def _build_files_tab(self):
        ...
        ttk.Label(info, text="Рекомендации по формату CSV", style="H2.TLabel").pack(anchor="w")
        ttk.Label(
            info,
            text=(
                "• Программа принимает только файлы формата CSV.\n"
                "• Рекомендуется наличие колонки времени, однако она не является обязательной.\n"
                "• Если колонка времени отсутствует, временная шкала формируется автоматически на основе заданной частоты дискретизации (FS).\n"
                "• CSV-файл должен содержать одну или несколько числовых колонок ЭЭГ-сигнала (каналы).\n\n"
                "Условия работы режима сравнения:\n"
                "• Сравнение выполняется только при наличии двух и более файлов.\n"
                "• Во всех файлах должно быть одинаковое количество сигнальных колонок (без учёта колонки времени).\n"
                "• Частота дискретизации (FS) должна быть одинаковой для всех сравниваемых записей.\n\n"
                "Если хотя бы одно из условий не выполняется, программа автоматически отключает режим сравнения\n"
                "и выполняет анализ каждого файла отдельно, с явным указанием причины в отчёте."
            ),
            style="Muted.TLabel",
            justify="left",
            wraplength=620,
